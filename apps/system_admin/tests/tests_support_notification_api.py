...
class AdminSupportNotificationApiTests(TestCase):
    ...
    def test_support_ticket_workflow(self):
        ...
        list_response = self.client.get(reverse("system_admin:support-ticket-list"), {"status": "open"})
        ...
        detail_response = self.client.get(reverse("system_admin:support-ticket-detail", kwargs={"pk": self.ticket.pk}))
        ...
        reply_response = self.client.post(
            reverse("system_admin:support-ticket-messages", kwargs={"pk": self.ticket.pk}),
        ...
        escalate_response = self.client.post(
            reverse("system_admin:support-ticket-escalate", kwargs={"pk": self.ticket.pk}),
        ...
        resolve_response = self.client.post(
            reverse("system_admin:support-ticket-resolve", kwargs={"pk": self.ticket.pk}),
        ...
        stats_response = self.client.get(reverse("system_admin:support-ticket-statistics"))
    ...
    def test_notification_workflow_publish_export_and_constraints(self):
        ...
        self.assertEqual(patch_response.data["data"]["title"], "Nhắc lịch tham gia sự kiện")

        publish_response = self.client.post(
            reverse("system_admin:notification-publish", kwargs={"pk": notification_id}),
            format="json",
        )
        ...
        blocked_delete = self.client.delete(reverse("system_admin:notification-detail", kwargs={"pk": notification_id}))
        ...
        excel_response = self.client.get(reverse("system_admin:notification-export"), {"export_format": "xlsx"})
    ...
    def test_notification_scheduled_statistics_and_permissions(self):
        ...
        self.assert_success_envelope(response, expected_status=201, expected_code=ResponseCode.CREATED.value)

        stats_response = self.client.get(reverse("system_admin:notification-statistics"))
        self.assert_success_envelope(stats_response)
        self.assertGreaterEqual(stats_response.data["data"]["scheduled"], 1)

        config_response = self.client.get(reverse("system_admin:notification-pagination-config"))
    ...
    def test_notification_rejects_past_schedule_time(self):
        ...
        patch_response = self.client.patch(
            reverse("system_admin:notification-detail", kwargs={"pk": draft_response.data["data"]["id"]}),
            {
                "status": "scheduled",
                "scheduled_at": past_scheduled_at.isoformat(),
            },
    ...
    def test_due_scheduled_notifications_are_published_by_scheduler_service(self):
        ...
        result = AdminNotificationService.publish_due_scheduled_notifications(batch_size=10)
