...
class RegistrationServiceTests(RegistrationTestMixin, APITestCase):
    ...
    def test_create_event_registration_allows_re_register_after_cancelled(self):
        ...
        ticket = self.create_ticket(registration, status_value=Ticket.TicketStatus.CANCELLED)
        ...
        self.assertEqual(registration.status, EventRegistration.RegistrationStatus.REGISTERED)
        self.assertEqual(registration.form_answers_jsonb, [{"fieldId": "dietary", "value": "Vegetarian"}])
    ...
    def test_create_event_registration_allows_re_register_after_rejected(self):
        registration = self.create_registration(status_value=EventRegistration.RegistrationStatus.REJECTED)
        ...
        self.assertTrue(Ticket.objects.filter(registration=registration, status=Ticket.TicketStatus.VALID).exists())
    ...
    def test_issue_registration_qr_creates_short_lived_token_payload(self):
        ...
        self.assertLessEqual((qr_data["valid_to"] - qr_data["valid_from"]).total_seconds(), 15)
    ...
    def test_process_checkin_scan_accepts_signed_short_lived_qr_token(self):
        ...
        self.assertEqual(ticket.status, Ticket.TicketStatus.USED)
        self.assertEqual(registration.status, EventRegistration.RegistrationStatus.CHECKED_IN)
        self.assertEqual(second_result["result"], CheckinLog.CheckinResult.ALREADY_CHECKED_IN)
...
class RegistrationApiTests(RegistrationTestMixin, APITestCase):
    ...
    def event_ticket_detail_url(self, event=None):
        return reverse("event-ticket-detail", kwargs={"event_id": (event or self.event).id})

    def event_ticket_token_url(self, event=None):
        return reverse("event-ticket-token", kwargs={"event_id": (event or self.event).id})
    ...
    def event_checkin_scan_url(self, event=None):
        return reverse("event-checkin-scan", kwargs={"event_id": (event or self.event).id})

    def event_checkin_list_url(self, event=None):
        return reverse("event-checkin-list", kwargs={"event_id": (event or self.event).id})

    def event_attendee_list_url(self, event=None):
        return reverse("event-attendee-list", kwargs={"event_id": (event or self.event).id})
    ...
    def test_ticket_detail_does_not_lookup_by_ticket_id(self):
        ...
        response = self.client.get(reverse("ticket-detail", kwargs={"registration_id": ticket.id}))
    ...
    def test_organizer_can_create_ticket_for_registration(self):
        ...
        response = self.client.post(self.ticket_detail_url(registration), {}, format="json")
    ...
    def test_organizer_can_update_ticket(self):
        ...
        response = self.client.patch(
            ...
            {"status": Ticket.TicketStatus.EXPIRED, "expires_at": expires_at.isoformat()},
            format="json",
        )
        ...
        self.assertEqual(ticket.expires_at.isoformat().replace("+00:00", "Z"), response.data["expires_at"])
    ...
    def test_organizer_can_scan_list_checkins_and_list_attendees(self):
        ...
        self.assertEqual(scan_response.data["ticket"]["ticket_code"], ticket.ticket_code)
        ticket.refresh_from_db()
        registration.refresh_from_db()
        self.assertEqual(ticket.status, Ticket.TicketStatus.USED)
        self.assertEqual(registration.status, EventRegistration.RegistrationStatus.CHECKED_IN)
        ...
        self.assertEqual(checkins_response.data["results"][0]["ticket"]["ticket_code"], ticket.ticket_code)
        ...
        self.assertEqual(attendees_response.data["results"][0]["id"], str(registration.id))
